...
class MLP(chainer.Chain):
    def __init__(self, n):
        ...
        self.N = n
        self.add_link('conv1', L.Convolution2D(in_channels=1,out_channels=64,ksize=(1,self.N)))
        self.add_link('conv2', L.Convolution2D(in_channels=64,out_channels=128,ksize=3,pad=1))
        self.add_link('conv3', L.Convolution2D(in_channels=128,out_channels=256,ksize=3,pad=1))
        self.add_link('conv4', L.Convolution2D(in_channels=256,out_channels=512,ksize=3,pad=1))
        self.add_link('bn1', L.BatchNormalization(128))
        self.add_link('bn2', L.BatchNormalization(512))
        self.add_link('l1', L.Linear(in_size=None,out_size=256))
        self.add_link('l2', L.Linear(in_size=256,out_size=n*n))

    def z(self, batch):
        return self.xp.random.randn(batch, self.channels[0]).astype('f')

    def __call__(self, x):
        x = x.reshape((-1,1,self.N,self.N))
        x = self['conv1'](x)
        x = self['conv2'](x)
        x = self['bn1'](x)
        x = F.relu(x)
        x = F.max_pooling_2d(x,ksize=3,stride=1,pad=1)
        x = self['conv3'](x)
        x = self['conv4'](x)
        x = self['bn2'](x)
        x = F.relu(x)
    ...
    for ep in range(1,epoch+1):
        ...
        while check:
            ...
            if epsilon > Mnet.xp.random.uniform(0,1,1):
                ...
                cmnn += 1
                mx = Mnet.xp.where(G[n*n:n*n*2] > -1,1,0)*Mnet(Mnet.xp.array([G[n*n:n*n*2]]).astype('f'))[0].data
                ep_check = 1
            if ep%100 == 0:
            ...
            if len(memory) >= pool_size:
                ...
                for i, [G_b,inputs_b,reward_b,next_G_b] in enumerate(minibatch):
                    ...
                    outputs = Mnet(Mnet.xp.array([G_b[n*n:n*n*2]]).astype('f'))[0].data
                    targets = outputs.copy()
                    next_outputs = target_Mnet(Mnet.xp.array([next_G_b[n*n:n*n*2]]).astype('f'))[0].data
