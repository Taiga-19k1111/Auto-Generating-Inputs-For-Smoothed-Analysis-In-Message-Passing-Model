...
class QNetwork:
    def __init__(self, state_size, action_size):
        ...
        self.model.compile(loss=Huber(), optimizer=Adam(learning_rate=0.0025))
...
def train():
    ...
    initial_G = gen_worstcase(n)
    initial_G,initial_post = gen_initial_graph_state(initial_G, n)
    ...
    for ep in range(1,epoch+1):
        ...
        while check:
            ...
            if total_step%1000 == 0:
                random_qn = QNetwork((n*n,), n*n)
                dummy_mx = random_qn.model.predict(np.array([initial_G[n*n:n*n*2]]))
                for i in range(4):
                    layer = main_qn.model.get_layer(index=i)
                    random_layer = random_qn.model.get_layer(index=i)
                    weights = layer.get_weights()
                    random_weights = random_layer.get_weights()
                    if i == 0:
                        layer.set_weights([weight*0.8 + random_weight*0.2 for weight, random_weight in zip(weights, random_weights)])
                    else:
                        layer.set_weights(random_weights)  
                main_qn.model.compile(loss=Huber(), optimizer=Adam(learning_rate=0.0025))              
        ...
        # if ep%10 == 0:
        #     random_qn = QNetwork((n*n,), n*n)
        #     dummy_G,_ = gen_initial_graph_state(initial_G, n)
        #     dummy_mx = random_qn.model.predict(np.array([dummy_G[n*n:n*n*2]]))
        #     for i in range(4):
        #         layer = main_qn.model.get_layer(index=i)
        #         random_layer = random_qn.model.get_layer(index=i)
        #         weights = layer.get_weights()
        #         random_weights = random_layer.get_weights()
        #         if i == 0:
        #             layer.set_weights(weights*0.8 + random_weights*0.2)
        #         else:
        #             layer.set_weights(random_weights)
