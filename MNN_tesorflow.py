    ...
    for ep in range(1,epoch+1):
        ...
        while check:
            ...
            if epsilon > cp.random.uniform(0,1,1):
                ...
                cmnn += 1
                mx = main_qn.model.predict(cp.asnumpy(cp.array([G[n*n:n*n*2]])))[0]
                ep_check = 1
            ...
            if len(memory) >= pool_size and total_step >= 1000:
                ...
                for i, [G_b,inputs_b,reward_b,next_G_b] in enumerate(minibatch):
                    ...
                    if next_initial_post.any():
                        next_q = cp.asarray(target_qn.model.predict(cp.asnumpy(cp.array([next_G_b[n*n:n*n*2]])))[0])
                        max_q = cp.amax(next_q*cp.where(next_initial_post,1,0))
                        target = reward_b + GAMMA*max_q
                    else:
                        target =reward_b
                    targets[i] = cp.asarray(main_qn.model.predict(cp.asnumpy(cp.array([inputs[i]])))[0])
