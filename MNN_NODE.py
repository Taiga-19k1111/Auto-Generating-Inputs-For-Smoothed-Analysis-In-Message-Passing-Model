    ...
    for ep in range(1,epoch+1):
        ...
        while check:
            ...
            if epsilon > Mnet.xp.random.uniform(0,1,1):
                ...
                cmnn += 1
                mx = Mnet.xp.zeros(n*n)
                for i in range(n):
                    for j in range(n):
                        if G[n*n+i*n+j] == -1:
                            continue
                        tmp = Mnet(Mnet.xp.array([G[n*n+i*n+j], G[n*n*2+i], G[n*n*2+j]]).astype('f'))[0].data
                        mx[i*n+j] = tmp
                ep_check = 1
            if ep%100 == 0:
            ...
            if step > start_training:
                memory.add([G,[inputs[0],inputs[1]],reward,next_G])
            ...
            if len(memory) >= pool_size:
                ...
                for a, [G_b,inputs_b,reward_b,next_G_b] in enumerate(minibatch):
                    i_b = inputs_b[0]
                    j_b = inputs_b[1]
                    output = Mnet(Mnet.xp.array([G_b[n*n+i_b*n+j_b], G_b[n*n*2+i_b], G_b[n*n*2+j_b]]).astype('f'))[0].data
                    next_memo = []
                    for i in range(n):
                        for j in range(n):
                            if next_G_b[n*n+i*n+j] == -1:
                                continue
                            tmp = target_Mnet(Mnet.xp.array([next_G_b[n*n+i*n+j], next_G_b[n*n*2+i], next_G_b[n*n*2+j]]).astype('f'))[0].data
                            next_memo.append(tmp)
                    next_memo = sorted(next_memo, reverse=True)
                    next_initial_post = (next_G_b[n*n:n*n*2] != -1)
                    if next_initial_post.any():
                        max_q = Mnet.xp.array(next_memo[0])
