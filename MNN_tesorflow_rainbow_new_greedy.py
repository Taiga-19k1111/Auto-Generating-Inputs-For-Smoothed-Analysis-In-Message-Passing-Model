...
class RainbowAgent:
    def __init__(self, n):
        ...
        self.explore_episodes = 100
        self.explore_switch = True
        self.epsilon = 0.9
        self.learning_times = 50000
    ...
    def learn(self, n_episodes):
        ...
        for ep in range(1,n_episodes+1):
            ...
            while check:
                ...
                if self.explore_switch:
                    if rnd < self.epsilon:
                        action = self.qnet.sample_action(state, mask_post)
                    else:
                        action = np.random.choice(np.where(G[self.n**2:(self.n**2)*2] != -1)[0])
                else:
                    qnet_action = self.qnet.sample_action(state, mask_post)
                    if rnd < self.epsilon:
                        actions = np.where(G[self.n**2:(self.n**2)*2] != -1)[0]
                        actions = actions[actions != qnet_action]
